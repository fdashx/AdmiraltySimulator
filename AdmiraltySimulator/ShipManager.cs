    ...
    public class ShipManager
    {
        ...
        public void LoadOneTimeUse(string file)
        {
            try
            {
                ...
                foreach (var line in File.ReadAllLines(file))
                {
                    ...
                    if (vals.Length != 2 || !int.TryParse(vals[0].Trim(), out var count))
                    {
        ...
        private bool TryParseShip(string shipInfo, out Ship ship)
        {
            ...
            if (!Enum.TryParse(vals[1].Trim(), true, out ShipType type)
                || !int.TryParse(vals[2].Trim(), out var engVal)
                || !int.TryParse(vals[3].Trim(), out var tacVal)
                || !int.TryParse(vals[4].Trim(), out var sciVal)
        ...
        private bool TryParseAbility(string abilityInfo, out IAbility ability)
        {
            ...
            if (!double.TryParse(vals[0], out var value))
                return false;
